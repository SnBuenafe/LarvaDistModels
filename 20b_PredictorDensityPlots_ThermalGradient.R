...
(ab_tunas2 <- plot_thermgrad(fin_tmp, spp, col_values) +
    theme(axis.title.x = element_blank()))

#### Plot the less abundant tunas ####
spp <- c("sbft", "bft", "lit", "slt", "bon")
col_values <- c("#48594B", "#708B75", "#91B397", "#A5CCAC", "#BAE6C1")
(lab_tunas2 <- plot_thermgrad(fin_tmp, spp, col_values) +
    theme(axis.title.x = element_blank()))

#### Plot the billfish ####
spp <- c("blum", "shos", "swo", "strm", "sail")
col_values <- c("#6A637A", "#9086A6", "#A297BA", "#B8ABD4", "#CABCE8")
(bill2 <- plot_thermgrad(fin_tmp, spp, col_values) + 
  theme(axis.title.x = element_blank()))

#### Plot the rest ####
spp <- c("lesc", "sau")
col_values <- c("#DB9F1D", "#FFD67D")
(oth2 <- plot_thermgrad(fin_tmp, spp, col_values))

all <- (ab_tunas2 / lab_tunas2 / bill2 / oth2) +
  plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")") &
  theme(plot.tag = element_text(size = 25))

ggsave(plot = all, filename = here::here(figure_dir, "ImptPredictors_thermal_gradient.png"), width = 15, height = 27, dpi = 300)
