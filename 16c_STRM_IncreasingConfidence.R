...
model <- readRDS(here::here(model_dir, "STRM_model1.rds")) # Load model we're using (here we're using the full model)
...
full_grid <- associateGrids(`grid_STRM_jan-mar`, grid_100)
...
ggsave(plot = gg1, filename = here::here(figure_dir, paste("STRM", "model1", "high_conf", "jan-mar.png", sep = "_")), width = 15, height = 7, dpi = 600)

#### April-June ####
# Associate seasonal grids with the 10x10 grid
full_grid <- associateGrids(`grid_STRM_apr-jun`, grid_100)
...
ggsave(plot = gg2, filename = here::here(figure_dir, paste("STRM", "model1", "high_conf", "apr-jun.png", sep = "_")), width = 15, height = 7, dpi = 600)

#### July-September ####
# Associate seasonal grids with the 10x10 grid
full_grid <- associateGrids(`grid_STRM_jul-sept`, grid_100)
...
ggsave(plot = gg3, filename = here::here(figure_dir, paste("STRM", "model1", "high_conf", "jul-sept.png", sep = "_")), width = 15, height = 7, dpi = 600)

#### October-December ####
# Associate seasonal grids with the 10x10 grid
full_grid <- associateGrids(`grid_STRM_oct-dec`, grid_100)
...
ggsave(plot = gg4, filename = here::here(figure_dir, paste("STRM", "model1", "high_conf", "oct-dec.png", sep = "_")), width = 15, height = 7, dpi = 600)

gg_full <- (gg1 + gg2) / (gg3 + gg4) +
  plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")") &
  theme(plot.tag = element_text(size = 25))

ggsave(plot = gg_full, filename = here::here(figure_dir, "STRM_model1_highconf.png"), width = 27, height = 15, dpi = 600)
