...
model <- readRDS(here::here(model_dir, "BLUM_model1.rds")) # Load model we're using (here we're using the full model)
...
full_grid <- associateGrids(`grid_BLUM_jan-mar`, grid_100)
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BLUM", "jan-mar.csv", sep = "_"))) # save predictions
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BLUM", "apr-jun.csv", sep = "_"))) # save predictions
...
#### July-September ####
# Associate seasonal grids with the 10x10 grid
full_grid <- associateGrids(`grid_BLUM_jul-sept`, grid_100)
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BLUM", "jul-sept.csv", sep = "_"))) # save predictions
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BLUM", "oct-dec.csv", sep = "_"))) # save predictions
...
ggsave(plot = gg_full, filename = here::here(figure_dir, "BLUM_model1_highconf.png"), width = 27, height = 15, dpi = 600)
