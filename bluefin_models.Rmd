---
title: "Building Pacific bluefin tuna models"
author: "Tin Buenafe"
...
First, we plot the presence/absence data from the Nishikawa dataset for bluefin tuna. The positive sampling points are confined to April-June and July-September.

### January-March

Note that for this season, there are no positive samples.
...
Most positive samples are south of Japan.
...
Not much positive sampling points. Those that are present are south of Japan.

```{r nish_js, echo = FALSE}
df <- `grid_BFT_jul-sept` %>%
  dplyr::select(cellID, abundance, geometry) %>%
  na.omit() %>%
  dplyr::mutate(abundance_presence = ifelse(abundance > 0, yes = 1, no = 0)) %>%
  sf::st_as_sf() %>%
...
```{r model1, echo = FALSE}
BFT_model1 <- readRDS("Output/Models/BFT_model1.rds")

print(paste0("training AUC: ", BFT_model1$self.statistics$discrimination)) # Training AUC Score
...
print(paste0("training AUC: ", BFT_model2$self.statistics$discrimination)) # Training AUC Score
...
gg1 <- plotModel(gg) + # Plot the squished model
    theme(legend.text = element_text(size = 8),
        legend.title = element_text(size = 10),
        plot.margin = unit(c(0,0.5,0,0.5), "cm"))
gg1 + nish1
