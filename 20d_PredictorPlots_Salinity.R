# DESCRIPTION: Summary plots for surface salinity
...
ggsave(plot = all, filename = here::here(figure_dir, "Seasonality_sos.png"), width = 30, height = 25, dpi = 300, limitsize = FALSE)
...
df %<>%
  dplyr::mutate(transformed = case_when(transformed < 33.5 ~ 33.5,
                                        transformed > 35.5 ~ 35.5,
                                        TRUE ~ transformed))

(ab_tunas <- plot_KD(df,
                      spp,
                      expression('Salinity (ppt)'),
                      limits = c(33.5, 35.5),
                      col_values
) +
...
ggsave(plot = all, filename = here::here(figure_dir, "ImptPredictors_sos.png"), width = 15, height = 27, dpi = 300)
