...
###########################################################################
# Load the model 3
SKP_model3 <- readRDS("Output/Models/SKP_model3.rds")
...
train_tmp <- train %>% 
  dplyr::mutate(model = SKP_model3$fitted)
...
gg <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                        ...
                        SKP_model3, # BRT model
                        `grid_SKP_jan-mar` %>% dplyr::filter(latitude >= min(train$latitude) & latitude <= max(train$latitude)) # grid of species for specific season with restricted ranges
)
...
ggseasons <- (ggseason1 + ggseason2) / (ggseason3 + ggseason4) + 
  ...
  plot_annotation("Model 3: Original predictors with best test AUC (AUC: 0.79)")

ggsave(plot = ggseasons, filename = "Figures/SKP/SKP_Model3.png", width = 27, height = 15, dpi = 600)

###########################################################################
## Model 4: Least overfitting in moderate trees (original predictors) ##
###########################################################################
# Load the model 4
SKP_model4 <- readRDS("Output/Models/SKP_model4.rds")
...
train_tmp <- train %>% 
  dplyr::mutate(model = SKP_model4$fitted)
...
ggseasons <- (ggseason1 + ggseason2) / (ggseason3 + ggseason4) + 
  plot_layout(guides = "collect") +
  plot_annotation("Model 4: Original predictors with least overfitting with moderate complexity (AUC: 0.79)")

ggsave(plot = ggseasons, filename = "Figures/SKP/SKP_Model4.png", width = 27, height = 15, dpi = 600)

###########################################################################
## Model 6: Best test AUC (additional predictors) ##
###########################################################################
# Load the model 6
SKP_model6 <- readRDS("Output/Models/SKP_model6.rds")
...
train_tmp <- train %>% 
  dplyr::mutate(model = SKP_model6$fitted)
...
ggseasons <- (ggseason1 + ggseason2) / (ggseason3 + ggseason4) + 
  plot_layout(guides = "collect") +
  plot_annotation("Model 6: Additional predictors with best test AUC (AUC: 0.80)")

ggsave(plot = ggseasons, filename = "Figures/SKP/SKP_Model6.png", width = 27, height = 15, dpi = 600)

#### Plot test vs predictors ####
pdf(file = "Figures/SKP/SKP_Model6_PredictorsTrain.pdf", width = 10, height = 8)
gbm.plot.fits(SKP_model6)
dev.off()
...
ggsave(filename = "Figures/SKP/SKP_Model6_PredictorsTest.pdf", plot = ggpredictors, width = 12, height = 8, dpi = 300)

###########################################################################
## Model 7: Least overfitting w/ moderate complexity (additional predictors) ##
###########################################################################
# Load the model 7
SKP_model7 <- readRDS("Output/Models/SKP_model7.rds")
...
train_tmp <- train %>% 
  dplyr::mutate(model = SKP_model7$fitted)
...
ggseasons <- (ggseason1 + ggseason2) / (ggseason3 + ggseason4) + 
  plot_layout(guides = "collect") +
  plot_annotation("Model 7: Least overfitting w/ moderate complexity (AUC: 0.80)")

ggsave(plot = ggseasons, filename = "Figures/SKP/SKP_Model7.png", width = 27, height = 15, dpi = 600)