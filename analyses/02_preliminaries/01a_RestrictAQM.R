# DESCRIPTION: Restrict final models to the adults' AquaMaps range
...
# Restricting the data set to the species that are included in the analysis
# If there are matches with multiple model species we need to merge the columns.
# We're taking the highest probability across all the model species per taxon, rather than the mean or the sum.
...
aq <- readRDS("data_input/AquaMaps_sf.rds") %>% 
  ...
  mutate(across(!geometry, ~ if_else(.x >= 0.01, .x, NA))) %>% # if >0.01 -> presence
...
aq <- aq %>% 
  ...
  dplyr::mutate(Binary = 1) # change to binary
...
range_limits <- aq2 %>% 
  ...
  summarise(MinLon = min(Longitude), # Need to do it in two sets to deal with dateline
...
write_rds(range_limits, file.path("data_output", "RangeLimits.rds"))
