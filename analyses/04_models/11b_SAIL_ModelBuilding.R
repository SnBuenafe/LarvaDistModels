...
SAIL_model <- dismo::gbm.step(data = train, gbm.x = brt_cols,
                               gbm.y = 5, family = "bernoulli", n.folds = 5,
                               tree.complexity = 2, bag.fraction = 0.75, learning.rate = 0.01
)
saveRDS(SAIL_model, here::here(model_dir, paste(species, "model.rds", sep = "_")))
# SAIL_model <- readRDS(here::here(model_dir, paste(species, "model.rds", sep = "_")))
...
# Printing AUCs
SAIL_model$self.statistics$discrimination # Training AUC Score
SAIL_model$cv.statistics$discrimination.mean # Validation AUC Score

# Predict to the testing dataset
preds <- gbm::predict.gbm(SAIL_model, test, n.trees = SAIL_model$gbm.call$best.trees, type = "response")
dismo::calc.deviance(test[, "abundance_presence"], preds, family = "bernoulli")
get_testAUC(test$abundance_presence, preds) # Print testing AUC

#### Plotting maps ####
train_tmp <- train %>% 
  dplyr::mutate(model = SAIL_model$fitted)
test_tmp <- test %>% 
...
gg_obj <- create_speciesMap(train_tmp, # training object with model column (fitted values)
                            ...
                            SAIL_model, # BRT model
                            `grid_SAIL_jul-sep` # grid of species for specific season
)
