...
BFT_model <- dismo::gbm.step(data = train, gbm.x = brt_cols,
...
saveRDS(BFT_model, file.path(model_dir, paste(species, "model.rds", sep = "_")))
# BFT_model <- readRDS(file.path(model_dir, paste(species, "model.rds", sep = "_")))
...
gg_obj <- create_speciesMap(train_tmp, # training object with model column (fitted values)
                            ...
                            BFT_model, # BRT model
                            `grid_BFT_jul-sep` # grid of species for specific season
)

gg <- plotModel(gg_obj, limits) # plot extrapolation on jul-sep
