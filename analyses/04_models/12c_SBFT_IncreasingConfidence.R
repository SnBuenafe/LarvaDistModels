...
# Plot the longitude and latitude bar plots
bps <- plotLonLat(gg_filt, full_grid)
ggsave(plot = bps$longitude, filename = here::here(figure_dir, paste(species, "longitude", "apr-mar.png", sep = "_")), width = 7, height = 0.5, dpi = 600)
ggsave(plot = bps$latitude, filename = here::here(figure_dir, paste(species, "latitude", "apr-mar.png", sep = "_")), width = 5, height = 1, dpi = 600)

#### July-September ####
# Associate seasonal grids with the 10x10 grid
full_grid <- associateGrids(`grid_SBFT_jul-sep`, grid_100)
...
gg_obj <- create_speciesMap(train_tmp, # training object with model column (fitted values)
                            ...
                            model, # BRT model
                            `grid_SBFT_jul-sep` # grid of species for specific season
)
