...
for(i in 1:length(spp_list)) {
  obj <- readRDS(file.path(preds_dir, paste(toupper(spp_list[i]), "jul-sep.rds", sep = "_")))
...
PC_scores <- PCA$scores[,1:2] %>% # first two axes
  tibble::as_tibble()
write.csv(PC_scores, file = file.path(pc_dir, "hotspots_jul-sep_scores.csv"))
# PC_scores <- read_csv(file.path(pc_dir, "hotspots_jul-sep_scores.csv"))

loadings <- PCA$loadings %>% 
  as.data.frame.matrix()
write.csv(loadings, file = file.path(pc_dir, "hotspots_jul-sep_loadings.csv"))
...
sp_cor <- cor(cor_df %>% 
                dplyr::select(-cellID, -grid_100_category, -geometry),
              method = "spearman")
write.csv(sp_cor, file = file.path(pc_dir, "hotspots_jul-sep_spearmancorr.csv"))
...
ggsave(plot = pc1, filename = file.path(figure_dir, "PCA1_jul-sep.png"), width = 14, height = 5, dpi = 600)

# Plot PC2
pc2 <- plotPC2_limits(pc_obj, "Comp.2", "PC2 score  ")
ggsave(plot = pc2, filename = file.path(figure_dir, "PCA2_jul-sep.png"), width = 14, height = 5, dpi = 600)

#### Plotting scree plots ####
var_explained = PCA$sdev^2 / sum(PCA$sdev^2)
scree_gg <- plotScree(var_explained)

ggsave(plot = scree_gg, filename = file.path(figure_dir, "PCA_scree_jul-sep.png"), width = 7, height = 5, dpi = 600)
