# DESCRIPTION: Plotting potential models for SAIL
...
min(SAIL_build$latitude)
max(SAIL_build$latitude)
...
ggsquished <- (ggsquish1 + ggsquish2) / (ggsquish3 + ggsquish4) +
  plot_annotation("Best model (squished); AUC: 0.86")

ggsave(plot = ggsquished, filename = "Figures/SAIL/SAIL_model2_squished.png", width = 27, height = 15, dpi = 600)

#### Plot relative importance of variables ####
rel_imp <- summary(SAIL_model2)
