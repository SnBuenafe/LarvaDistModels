# DESCRIPTION: Summary plots for ammonium concentration
...
fin_tmp <- assemblePreds("nh4os") %>% 
  dplyr::mutate(across(everything(), ~(.x*1000))) # convert to mmol m-3 instead of mol m-3
...
(skp <- plotBP(fin_tmp,
               ...
               "Skipjack tuna") +
    ylab(expression('Ammonium concentration (mmol m'^'-3'*')')) +
...
ggsave(plot = all, filename = here::here(figure_dir, "nh4os_Seasonality.png"), width = 60, height = 20, dpi = 300, limitsize = FALSE)
...
df %<>%
  ...
  dplyr::mutate(transformed = case_when(transformed > 0 ~ 0,
                                        TRUE ~ transformed))

(ab_tunas <- plot_KD(df,
                     spp,
                     expression('log (ammonium)'),
                     limits = c(NA, 0),
                     col_values
) +
...
ggsave(plot = all, filename = here::here(figure_dir, "ImptPredictors_nh4os.png"), width = 15, height = 27, dpi = 300)
