...
# Yellowfin tuna
obj <- readRDS(here::here(preds_dir, "YFT_jan-mar.rds"))
yft <- prepare_components(obj, "yft")

# Skipjack tuna
obj <- readRDS(here::here(preds_dir, "SKP_jan-mar.rds"))
skp <- prepare_components(obj, "skp")

# Albacore
obj <- readRDS(here::here(preds_dir, "ALB_jan-mar.rds"))
alb <- prepare_components(obj, "alb")

# Swordfish
obj <- readRDS(here::here(preds_dir, "SWO_jan-mar.rds"))
swo <- prepare_components(obj, "swo")

# Blue marlin
obj <- readRDS(here::here(preds_dir, "BLUM_jan-mar.rds"))
blum <- prepare_components(obj, "blum")

# Frigate tuna
obj <- readRDS(here::here(preds_dir, "FRI_jan-mar.rds"))
fri <- prepare_components(obj, "fri")

# Bigeye tuna
obj <- readRDS(here::here(preds_dir, "BET_jan-mar.rds"))
bet <- prepare_components(obj, "bet")

# Pacific bluefin tuna
obj <- readRDS(here::here(preds_dir, "BFT_jan-mar.rds"))
bft <- prepare_components(obj, "bft")

# Sauries
obj <- readRDS(here::here(preds_dir, "SAU_jan-mar.rds"))
sau <- prepare_components(obj, "sau")

# Sailfish
obj <- readRDS(here::here(preds_dir, "SAIL_jan-mar.rds"))
sail <- prepare_components(obj, "sail")

# Southern bluefin tuna
obj <- readRDS(here::here(preds_dir, "SBFT_jan-mar.rds"))
sbft <- prepare_components(obj, "sbft")

# Slender tuna
obj <- readRDS(here::here(preds_dir, "SLT_jan-mar.rds"))
slt <- prepare_components(obj, "slt")

# Bonitos
obj <- readRDS(here::here(preds_dir, "BON_jan-mar.rds"))
bon <- prepare_components(obj, "bon")

# Shortbill spearfish
obj <- readRDS(here::here(preds_dir, "SHOS_jan-mar.rds"))
shos <- prepare_components(obj, "shos")

# Striped marlin
obj <- readRDS(here::here(preds_dir, "STRM_jan-mar.rds"))
strm <- prepare_components(obj, "strm")

# Longfin escolar
obj <- readRDS(here::here(preds_dir, "LESC_jan-mar.rds"))
lesc <- prepare_components(obj, "lesc")

# Little tuna
obj <- readRDS(here::here(preds_dir, "LIT_jan-mar.rds"))
...
write.csv(df, file = here::here(preds_dir, "FULL_predictions_jan-mar.csv"))
...
write.csv(loadings, file = here::here(pc_dir, "hotspots_jan-mar_loadings.csv"))
...
ggsave(plot = pc_plot, filename = here::here(figure_dir, "PC_plot_jan-mar.png"), width = 35, height = 7.5, dpi = 300)
