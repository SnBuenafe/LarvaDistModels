...
restrict_predictor <- function(x){
  x %<>%
    dplyr::select(c(1:19, 21:22, 36:38, 40)) %>%  # restrict the predictors
    rowwise() %>% 
...
SAU_ds1 <- read_csv("Output/CSV/SAU_historical_jan-mar.csv", show_col_types = FALSE) %>% # January-March
  restrict_predictor()
SAU_ds2 <- read_csv("Output/CSV/SAU_historical_apr-jun.csv", show_col_types = FALSE) %>% # April-June
  restrict_predictor()
SAU_ds3 <- read_csv("Output/CSV/SAU_historical_jul-sept.csv", show_col_types = FALSE) %>% # July-September
  restrict_predictor()
SAU_ds4 <- read_csv("Output/CSV/SAU_historical_oct-dec.csv", show_col_types = FALSE) %>% # October-December
  restrict_predictor()
...
nrow(SAU_build) * 0.9 # = 11051.1

set.seed(4411) # rerun this
train <- slice_sample(SAU_build, n = 11051, replace = FALSE) # 90% training set
