...
BET_model1 <- dismo::gbm.step(data = train, gbm.x = c(9:23, 26),
                              gbm.y = 5, family = "bernoulli", n.folds = 5,
                              tree.complexity = 2, bag.fraction = 0.5, learning.rate = 0.005
)
saveRDS(BET_model1, "Output/Models/BET_model1.rds")
# BET_model1 <- readRDS("Output/Models/BET_model1.rds")
...
ggsquish1 <- plotSquishedModel(gg[[1]], BET_ds1) # Plot the squished model
...
ggsquish2 <- plotSquishedModel(gg[[1]], BET_ds2) # Plot the squished model
...
ggsquish3 <- plotSquishedModel(gg[[1]], BET_ds3) # Plot the squished model
...
ggsquish4 <- plotSquishedModel(gg[[1]], BET_ds4) # Plot the squished model

ggsquished <- (ggsquish1 + ggsquish2) / (ggsquish3 + ggsquish4) +
  plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")") &
  theme(plot.tag = element_text(size = 25))

ggsave(plot = ggsquished, filename = "Figures/BET/BET_model1_squished_hatched.png", width = 27, height = 15, dpi = 600)
