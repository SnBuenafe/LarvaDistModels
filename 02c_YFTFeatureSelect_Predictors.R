...
ggseasons <- (ggseason1 + ggseason2) / (ggseason3 + ggseason4) + 
  ...
  plot_annotation("Model 4: New Predictors (AUC: 0.79)")
...
gg <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                        ...
                        `grid_YFT_jan-mar` %>% dplyr::filter(latitude >= min(train$latitude) & latitude <= max(train$latitude)) # grid of species for specific season
)
