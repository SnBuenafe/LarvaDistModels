...
print(CVGrid %>% dplyr::filter(tree_complexity == 3) %>% dplyr::arrange(train_test_diff)) # LEAST OVERFIT IN DEEPEST TREES

YFT_model6 <- dismo::gbm.step(data = train, gbm.x = c(7:13, 19:20),
                              gbm.y = 5, family = "bernoulli", n.folds = 5,
                              tree.complexity = 3, bag.fraction = 0.5, learning.rate = 0.005
                              )
...
############################################
print(CVGrid %>% dplyr::filter(tree_complexity == 2 & learning_rate == 0.01) %>% dplyr::arrange(test_AUC)) 
...
###########################################
print(CVGrid %>% dplyr::filter(tree_complexity == 2) %>% dplyr::arrange(desc(test_AUC)))
...
print(CVGrid %>% dplyr::filter(tree_complexity == 2) %>% dplyr::arrange(train_test_diff)) # LEAST OVERFIT
