# DESCRIPTION: Summary plots for mixed layer thickness
...
fin_tmp <- assemblePreds("mlotst")

#### Boxplots ####
breaks = seq(10, 120, 25)
limits = c(10, 120)
expand = c(0,0)
...
ggsave(plot = all, filename = here::here(figure_dir, "mlotst_Seasonality.png"), width = 60, height = 20, dpi = 300, limitsize = FALSE)
...
df %<>%
  dplyr::mutate(transformed = case_when(transformed < 10 ~ 10,
                                        transformed > 100 ~ 100,
                                        TRUE ~ transformed))

(ab_tunas <- plot_KD(df,
                     spp,
                     expression('Mixed layer thickness (m)'),
                     limits = c(10, 100),
                     col_values
) +
...
ggsave(plot = all, filename = here::here(figure_dir, "ImptPredictors_mlotst.png"), width = 15, height = 27, dpi = 300)
