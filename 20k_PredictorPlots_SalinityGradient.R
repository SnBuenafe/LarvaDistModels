# DESCRIPTION: Summary plots for broad-scale salinity gradients
...
fin_tmp <- assemblePreds("salinity_front")

#### Boxplots ####
breaks = waiver()
limits = c(0, 0.004)
expand = c(0.0005,0.0005)

(skp <- plotBP(fin_tmp,
               "skp", 
               breaks, limits, expand,
               "Skipjack tuna") +
    ylab(expression('Salinity gradient (Δ ppt km'^"-1"*')')) +
...
ggsave(plot = all, filename = here::here(figure_dir, "Seasonality_SalinityGradient.png"), width = 30, height = 25, dpi = 300, limitsize = FALSE)
...
(ab_tunas <- plot_KD(df,
                     spp,
                     expression('log (salinity gradient)'),
                     limits = c(NA, NA),
                     col_values
) +
...
ggsave(plot = all, filename = here::here(figure_dir, "ImptPredictors_SalinityGradient.png"), width = 15, height = 27, dpi = 300)
