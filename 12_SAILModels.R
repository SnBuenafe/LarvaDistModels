...
## Load dataset ##
###########################
# Load sailfish full dataset
SAIL_ds <- read_csv("Output/SAIL_full.csv", show_col_types = FALSE)

# Always build models with known data only
SAIL_filtered <- SAIL_ds %>% 
...
colnames(SAIL_filtered)
###########################
## Grid search ##
###########################
# Do a cross-validated grid search using gbm.fixed
# define a 5-fold cross-validation
CVGrid <- CVgridSearch(SAIL_filtered, 5, tc = c(1, 2, 3, 5), bf = c(0.5, 0.75), lr = 0.005)
...
SAIL_model1 <- gbm.step(SAIL_filtered, gbm.x = c(4, 6, 8:13), gbm.y = 14, 
                       learning.rate = 0.005,
                       bag.fraction = 0.5,
                       tree.complexity = 5
)
saveRDS(SAIL_model1, "Output/SAIL_model1.rds") # save the model
SAIL_model1 <- readRDS("Output/SAIL_model1.rds") # load the model

summary(SAIL_model1) # get the relative importance of each of the predictors

# Plot predictors
pdf(file = "Figures/SAIL_Model1_SmoothPredictors.pdf", width = 10, height = 8)
gbm.plot(SAIL_model1, n.plots=8, plot.layout=c(3, 3), write.title = FALSE)
dev.off()

pdf(file = "Figures/SAIL_Model1_Predictors.pdf", width = 10, height = 8)
gbm.plot.fits(SAIL_model1)
dev.off()
...
SAIL_model1$cv.statistics$discrimination.mean # AUC Score

# Check the plot
SAIL_filtered$model <- SAIL_model1$fitted
SAIL_sf <- grid %>% # convert to sf so we can plot
  dplyr::left_join(SAIL_filtered, ., by = "cellID") %>% 
...
###############################
preds <- dismo::predict(SAIL_model1, SAIL_predict, n.trees = SAIL_model1$gbm.call$best.trees, type = "response")
SAIL_predict$predictions <- preds

SAIL_predict_sf <- grid_SAIL %>% # convert to sf so we can plot
  dplyr::select(-species, -abundance, -season, -longitude, -latitude) %>% 
  dplyr::left_join(SAIL_predict, ., by = "cellID") %>% 
  sf::st_as_sf(sf_column_name = "geometry")

plotPredictions(SAIL_predict_sf, "Figures/SAIL_Model1preds.png") # Plot predictions
