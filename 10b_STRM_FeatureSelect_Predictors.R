...
STRM_model1 <- dismo::gbm.step(data = train, gbm.x = c(7:20),
                              gbm.y = 5, family = "bernoulli", n.folds = 5)
saveRDS(STRM_model1, "Output/Models/STRM_model1.rds") # save the model
#STRM_model1 <- readRDS("Output/Models/STRM_model1.rds") # load the model
...
gg <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                        ...
                        `grid_STRM_jan-mar` %>% dplyr::filter(latitude >= min(STRM_build$latitude) & latitude <= max(STRM_build$latitude)) # grid of species for specific season
)
...
ggseasons <- (ggseason1 + ggseason2) / (ggseason3 + ggseason4) + 
  ...
  plot_annotation("Model 1: Additional Predictors (AUC: 0.79)")

ggsave(plot = ggseasons, filename = "Figures/STRM/STRM_model1.png", width = 27, height = 15, dpi = 600)
