...
STRM_model1 <- dismo::gbm.step(data = train, gbm.x = c(9:23, 26),
                              gbm.y = 5, family = "bernoulli", n.folds = 5,
                              tree.complexity = 2, bag.fraction = 0.5, learning.rate = 0.008
)
saveRDS(STRM_model1, "Output/Models/STRM_model1.rds")
# STRM_model1 <- readRDS("Output/Models/STRM_model1.rds")
...
preds <- gbm::predict.gbm(STRM_model1, test, n.trees = STRM_model1$gbm.call$best.trees, type = "response") # predict to test
...
ggsquish1 <- plotSquishedModel(gg[[1]], STRM_ds1) # Plot the squished model
...
ggsquish2 <- plotSquishedModel(gg[[1]], STRM_ds2) # Plot the squished model
...
ggsquish3 <- plotSquishedModel(gg[[1]], STRM_ds3) # Plot the squished model
...
ggsquish4 <- plotSquishedModel(gg[[1]], STRM_ds4) # Plot the squished model

ggsquished <- (ggsquish1 + ggsquish2) / (ggsquish3 + ggsquish4) +
  plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")") &
  theme(plot.tag = element_text(size = 25))

ggsave(plot = ggsquished, filename = "Figures/STRM/STRM_model1_squished_hatched.png", width = 27, height = 15, dpi = 600)
