...
yft <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_YFT_apr-jun` %>% dplyr::filter(latitude >= min(YFT_build$latitude) & latitude <= max(YFT_build$latitude)) # grid of species for specific season with restricted ranges
)
...
skp <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_SKP_apr-jun` %>% dplyr::filter(latitude >= min(SKP_build$latitude) & latitude <= max(SKP_build$latitude)) # grid of species for specific season with restricted ranges
)
...
alb <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_ALB_apr-jun` %>% dplyr::filter(latitude >= min(ALB_build$latitude) & latitude <= max(ALB_build$latitude)) # grid of species for specific season with restricted ranges
)
...
swo <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_SWO_apr-jun` %>% dplyr::filter(latitude >= min(SWO_build$latitude) & latitude <= max(SWO_build$latitude)) # grid of species for specific season with restricted ranges
)
...
blum <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_BLUM_apr-jun` %>% dplyr::filter(latitude >= min(BLUM_build$latitude) & latitude <= max(BLUM_build$latitude)) # grid of species for specific season with restricted ranges
)
...
shos <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_SHOS_apr-jun` %>% dplyr::filter(latitude >= min(SHOS_build$latitude) & latitude <= max(SHOS_build$latitude)) # grid of species for specific season with restricted ranges
)
...
fri <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_FRI_apr-jun` %>% dplyr::filter(latitude >= min(FRI_build$latitude) & latitude <= max(FRI_build$latitude)) # grid of species for specific season with restricted ranges
)
...
bet <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_BET_apr-jun` %>% dplyr::filter(latitude >= min(BET_build$latitude) & latitude <= max(BET_build$latitude)) # grid of species for specific season with restricted ranges
)
...
strm <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_STRM_apr-jun` %>% dplyr::filter(latitude >= min(STRM_build$latitude) & latitude <= max(STRM_build$latitude)) # grid of species for specific season with restricted ranges
)
...
sau <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_SAU_apr-jun` %>% dplyr::filter(latitude >= min(SAU_build$latitude) & latitude <= max(SAU_build$latitude)) # grid of species for specific season with restricted ranges
)
...
sail <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_SAIL_apr-jun` %>% dplyr::filter(latitude >= min(SAIL_build$latitude) & latitude <= max(SAIL_build$latitude)) # grid of species for specific season with restricted ranges
)
...
lesc <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_LESC_apr-jun` %>% dplyr::filter(latitude >= min(LESC_build$latitude) & latitude <= max(LESC_build$latitude)) # grid of species for specific season with restricted ranges
)
...
write.csv(df, file = "Output/CSV/FULL_predictions_apr-jun.csv")
...
PC_scores <- PCA$scores[,1:2] %>% 
  tibble::as_tibble()
write.csv(PC_scores, file = "Output/PCA/hotspots_apr-jun_scores.csv")
write.csv(PCA$loadings, file = "Output/PCA/hotspots_apr-jun_loadings.csv")
