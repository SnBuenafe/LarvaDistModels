...
(ab_tunas5 <- plot_chl(fin_tmp, spp, col_values) +
  theme(axis.title.x = element_blank()))

### Plot the less abundant tunas ###
spp <- c("sbft", "bft", "lit", "slt", "bon")
col_values <- c("#48594B", "#708B75", "#91B397", "#A5CCAC", "#BAE6C1")
(lab_tunas5 <- plot_chl(fin_tmp, spp, col_values) +
    theme(axis.title.x = element_blank()))

#### Plot the billfish ####
spp <- c("blum", "shos", "swo", "strm", "sail")
col_values <- c("#6A637A", "#9086A6", "#A297BA", "#B8ABD4", "#CABCE8")
(bill5 <- plot_chl(fin_tmp, spp, col_values) +
  theme(axis.title.x = element_blank()))

#### Plot the rest ####
spp <- c("lesc", "sau")
col_values <- c("#DB9F1D", "#FFD67D")
(oth5 <- plot_chl(fin_tmp, spp, col_values))

all <- (ab_tunas5 / lab_tunas5 / bill5 / oth5) +
