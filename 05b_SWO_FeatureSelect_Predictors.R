...
SWO_model1 <- dismo::gbm.step(data = train, gbm.x = c(7:21, 23:24),
...
gg <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                        ...
                        `grid_SWO_jan-mar` %>% dplyr::filter(latitude >= min(SWO_build$latitude) & latitude <= max(SWO_build$latitude)) # grid of species for specific season
)
...
ggseasons <- (ggseason1 + ggseason2) / (ggseason3 + ggseason4) + 
  plot_annotation("Model 1: Additional Predictors (AUC: 0.78)")
