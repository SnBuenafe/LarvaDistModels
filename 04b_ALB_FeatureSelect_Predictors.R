...
ALB_model1 <- dismo::gbm.step(data = train, gbm.x = c(7:21, 23:24),
...
gg <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                        ...
                        `grid_ALB_jan-mar` %>% dplyr::filter(latitude >= min(ALB_build$latitude) & latitude <= max(ALB_build$latitude)) # grid of species for specific season
)
