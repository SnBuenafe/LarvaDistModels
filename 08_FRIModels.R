...
FRI_ds <- read_csv("Output/FRI_full.csv", show_col_types = FALSE)
...
CVGrid <- CVgridSearch(FRI_filtered, 5, tc = c(1, 2, 3, 5), bf = c(0.5, 0.75), lr = 0.005)
...
FRI_model1 <- readRDS("Output/FRI_model1.rds") # load the model

summary(FRI_model1) # get the relative importance of each of the predictors

# Plot predictors
pdf(file = "Figures/FRI_Model1_SmoothPredictors.pdf", width = 10, height = 8)
gbm.plot(FRI_model1, n.plots=8, plot.layout=c(3, 3), write.title = FALSE)
dev.off()

pdf(file = "Figures/FRI_Model1_Predictors.pdf", width = 10, height = 8)
...
# Check the plot
FRI_filtered$model <- FRI_model1$fitted
FRI_sf <- grid %>% # convert to sf so we can plot
  dplyr::left_join(FRI_filtered, ., by = "cellID") %>% 
...
###############################
preds <- dismo::predict(FRI_model1, FRI_predict, n.trees = FRI_model1$gbm.call$best.trees, type = "response")
FRI_predict$predictions <- preds

FRI_predict_sf <- grid_FRI %>% # convert to sf so we can plot
  dplyr::select(-species, -abundance, -season, -longitude, -latitude) %>% 
  dplyr::left_join(FRI_predict, ., by = "cellID") %>% 
  sf::st_as_sf(sf_column_name = "geometry")

plotPredictions(FRI_predict_sf, "Figures/FRI_Model1preds.png") # Plot predictions
