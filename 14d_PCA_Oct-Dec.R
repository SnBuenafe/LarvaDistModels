...
skp <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_SKP_oct-dec` %>% dplyr::filter(latitude >= min(SKP_build$latitude) & latitude <= max(SKP_build$latitude)) # grid of species for specific season with restricted ranges
)
...
alb <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_ALB_oct-dec` %>% dplyr::filter(latitude >= min(ALB_build$latitude) & latitude <= max(ALB_build$latitude)) # grid of species for specific season with restricted ranges
)
...
swo <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_SWO_oct-dec` %>% dplyr::filter(latitude >= min(SWO_build$latitude) & latitude <= max(SWO_build$latitude)) # grid of species for specific season with restricted ranges
)
...
blum <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_BLUM_oct-dec` %>% dplyr::filter(latitude >= min(BLUM_build$latitude) & latitude <= max(BLUM_build$latitude)) # grid of species for specific season with restricted ranges
)
...
shos <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_SHOS_oct-dec` %>% dplyr::filter(latitude >= min(SHOS_build$latitude) & latitude <= max(SHOS_build$latitude)) # grid of species for specific season with restricted ranges
)
...
fri <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_FRI_oct-dec` %>% dplyr::filter(latitude >= min(FRI_build$latitude) & latitude <= max(FRI_build$latitude)) # grid of species for specific season with restricted ranges
)
...
bet <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_BET_oct-dec` %>% dplyr::filter(latitude >= min(BET_build$latitude) & latitude <= max(BET_build$latitude)) # grid of species for specific season with restricted ranges
)
...
sau <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                         ...
                         `grid_SAU_oct-dec` %>% dplyr::filter(latitude >= min(SAU_build$latitude) & latitude <= max(SAU_build$latitude)) # grid of species for specific season with restricted ranges
)
...
sail <- plotSeasonPredict(train_tmp, # training object with model column (fitted values)
                          ...
                          `grid_SAIL_oct-dec` %>% dplyr::filter(latitude >= min(SAIL_build$latitude) & latitude <= max(SAIL_build$latitude)) # grid of species for specific season with restricted ranges
)
...
PC_scores <- PCA$scores[,1:2] %>% 
  tibble::as_tibble()
write.csv(PC_scores, file = "Output/PCA/hotspots_oct-dec_scores.csv")
write.csv(PCA$loadings, file = "Output/PCA/hotspots_oct-dec_loadings.csv")
