...
model <- readRDS(here::here(model_dir, "SKP_model1.rds")) # Load model we're using (here we're using the full model)
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("SKP", "jan-mar.csv", sep = "_"))) # save predictions
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("SKP", "apr-jun.csv", sep = "_"))) # save predictions
...
#### July-September ####
# Associate seasonal grids with the 10x10 grid
full_grid <- associateGrids(`grid_SKP_jul-sept`, grid_100)
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("SKP", "jul-sept.csv", sep = "_"))) # save predictions
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("SKP", "oct-dec.csv", sep = "_"))) # save predictions
...
ggsave(plot = gg_full, filename = here::here(figure_dir, "SKP_model1_highconf.png"), width = 27, height = 15, dpi = 600)
