...
restrict_predictor <- function(x){
  x %<>%
    dplyr::select(c(1:19, 21:22, 33:34, 40)) %>%  # restrict the predictors
    rowwise() %>% 
...
STRM_ds1 <- read_csv("Output/CSV/STRM_historical_jan-mar.csv", show_col_types = FALSE) %>% # January-March
  restrict_predictor()
STRM_ds2 <- read_csv("Output/CSV/STRM_historical_apr-jun.csv", show_col_types = FALSE) %>% # April-June
  restrict_predictor()
STRM_ds3 <- read_csv("Output/CSV/STRM_historical_jul-sept.csv", show_col_types = FALSE) %>% # July-September
  restrict_predictor()
STRM_ds4 <- read_csv("Output/CSV/STRM_historical_oct-dec.csv", show_col_types = FALSE) %>% # October-December
  restrict_predictor()
...
nrow(STRM_build) * 0.9 # = 11051.1

set.seed(3265) # rerun this
train <- slice_sample(STRM_build, n = 11051, replace = FALSE) # 90% training set
