...
# Yellowfin tuna
obj <- readRDS(here::here(pred_dir, "YFT_jul-sept.rds"))
yft <- prepare_components(obj, "yft")

# Skipjack tuna
obj <- readRDS(here::here(pred_dir, "SKP_jul-sept.rds"))
skp <- prepare_components(obj, "skp")

# Albacore
obj <- readRDS(here::here(pred_dir, "ALB_jul-sept.rds"))
alb <- prepare_components(obj, "alb")

# Swordfish
obj <- readRDS(here::here(pred_dir, "SWO_jul-sept.rds"))
swo <- prepare_components(obj, "swo")

# Blue marlin
obj <- readRDS(here::here(pred_dir, "BLUM_jul-sept.rds"))
blum <- prepare_components(obj, "blum")

# Frigate tuna
obj <- readRDS(here::here(pred_dir, "FRI_jul-sept.rds"))
fri <- prepare_components(obj, "fri")

# Bigeye tuna
obj <- readRDS(here::here(pred_dir, "BET_jul-sept.rds"))
bet <- prepare_components(obj, "bet")

# Pacific bluefin tuna
obj <- readRDS(here::here(pred_dir, "BFT_jul-sept.rds"))
bft <- prepare_components(obj, "bft")

# Sauries
obj <- readRDS(here::here(pred_dir, "SAU_jul-sept.rds"))
sau <- prepare_components(obj, "sau")

# Sailfish
obj <- readRDS(here::here(pred_dir, "SAIL_jul-sept.rds"))
sail <- prepare_components(obj, "sail")

# Southern bluefin tuna
obj <- readRDS(here::here(pred_dir, "SBFT_jul-sept.rds"))
sbft <- prepare_components(obj, "sbft")

# Slender tuna
obj <- readRDS(here::here(pred_dir, "SLT_jul-sept.rds"))
slt <- prepare_components(obj, "slt")

# Bonitos
obj <- readRDS(here::here(pred_dir, "BON_jul-sept.rds"))
bon <- prepare_components(obj, "bon")

# Shortbill spearfish
obj <- readRDS(here::here(pred_dir, "SHOS_jul-sept.rds"))
shos <- prepare_components(obj, "shos")

# Striped marlin
obj <- readRDS(here::here(pred_dir, "STRM_jul-sept.rds"))
strm <- prepare_components(obj, "strm")

# Longfin escolar
obj <- readRDS(here::here(pred_dir, "LESC_jul-sept.rds"))
lesc <- prepare_components(obj, "lesc")

# Little tuna
obj <- readRDS(here::here(pred_dir, "LIT_jul-sept.rds"))
...
write.csv(df, file = here::here(pred_dir, "FULL_predictions_jul-sept.csv"))
...
write.csv(PCA$loadings, file = here::here(pc_dir, "hotspots_jul-sept_loadings.csv"))
