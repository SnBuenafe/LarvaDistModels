...
# iv. July-September (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2017_2026_jul-sept_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2017, 2026, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_present_jul-sept_interpolated.rds")

# v. October-December (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2017_2026_oct-dec_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2017, 2026, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_present_oct-dec_interpolated.rds")
...
# ii. January-March (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2046_2055_jan-mar_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2046, 2055, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_midCentury_jan-mar_interpolated.rds")
# iii. April-June (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2046_2055_apr-jun_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2046, 2055, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_midCentury_apr-jun_interpolated.rds")
# iv. July-September (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2046_2055_jul-sept_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2046, 2055, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_midCentury_jul-sept_interpolated.rds")
# v. October-December (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2046_2055_oct-dec_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2046, 2055, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_midCentury_oct-dec_interpolated.rds")
...
# ii. January-March (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2091_2100_jan-mar_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2091, 2100, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_endCentury_jan-mar_interpolated.rds")
# iii. April-June (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2091_2100_apr-jun_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2091, 2100, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_endCentury_apr-jun_interpolated.rds")
# iv. July-September (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2091_2100_jul-sept_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2091, 2100, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_endCentury_jul-sept_interpolated.rds")
# v. October-December (seasonal)
tos_rs <- stars::read_ncdf("Data/Climatology/ensemble/tos_ssp585_2091_2100_oct-dec_ensemble.nc") %>% 
  terra::rast()
names(tos_rs) <- paste0("X", seq(2091, 2100, by = 1))
tos <- rs2sf(tos_rs) %>% 
  dplyr::rename(tos = mean) %>% # using the mean of the models
  ...
  dplyr::as_tibble() %>% 
  dplyr::select(cellID, tos_transformed, geometry)
saveRDS(tos, "Data/Climatology/sf/tos_endCentury_oct-dec_interpolated.rds")