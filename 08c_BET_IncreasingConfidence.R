...
model <- readRDS(here::here(model_dir, "BET_model1.rds")) # Load model we're using (here we're using the full model)
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BET", "jan-mar.csv", sep = "_"))) # save predictions
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BET", "apr-jun.csv", sep = "_"))) # save predictions
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BET", "jul-sept.csv", sep = "_"))) # save predictions
...
gg_filt <- restrictThreshold(full_grid,
                             gg,
                             10)
saveRDS(object = gg_filt, file = here::here(preds_dir, paste("BET", "oct-dec.csv", sep = "_"))) # save predictions
...
ggsave(plot = gg_full, filename = here::here(figure_dir, "BET_model1_highconf.png"), width = 27, height = 15, dpi = 600)
